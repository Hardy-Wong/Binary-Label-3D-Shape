...
from src.feature.handle.select_position import select_positions


class AbsolutePositionAndDGMeshLinearVAE(nn.Module):
    """Positional and Deformation Gradient feature model compared to original MeshVAE.
    ...
    def __init__(
        ...
        sigma_max: float = 2.0,
        scale: float = 1e3,
        output_only_position: bool = False,
    ) -> None:
        super().__init__()
        ...
        self.scale = scale
        self.output_only_position = output_only_position
    ...
    def loss_function(self, *args, **kwargs) -> dict:
        ...
        input_positions = select_positions(input, num_of_f_in_1_node=12)
        recons_positions = select_positions(recons, num_of_f_in_1_node=12)
        batch_size = len(input_positions)
        positional_recon_errors = (
            torch.sqrt(
                torch.sum(
                    torch.square(
                        input_positions.reshape((batch_size, -1, 3))
                        - recons_positions.reshape((batch_size, -1, 3))
                    ),
                    dim=2,
                )
            )
        ...
        positional_recons_loss = torch.mean(positional_recon_errors)
        total_recon_errors = torch.sqrt(
            torch.sum(
                torch.square(
        ...
        total_recon_loss = torch.mean(total_recon_errors)
        ...
        pdb.set_trace()
        if self.output_only_position:
            loss = recon_weight * positional_recons_loss + kld_loss
            return {
                "loss": loss,
                "Reconstruction_Loss": positional_recons_loss.detach(),
                "KLD": kld_loss.detach(),
            }
        else:
            loss = recon_weight * total_recon_loss + kld_loss
            return {
                "loss": loss,
                "Reconstruction_Loss": positional_recons_loss.detach(),
                "KLD": kld_loss.detach(),
                "total_recon": total_recon_loss.detach(),
            }
    ...
    def generate(self, x: Tensor, **kwargs) -> Tensor:
        ...
        if self.output_only_position:
            ...
            return select_positions(recons, num_of_f_in_1_node=12)
